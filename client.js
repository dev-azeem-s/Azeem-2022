...
const setPublicKey = async (password) => {
    try {
        const pathExists = fs.existsSync(path.join(keyDir, 'public.pem'));
        if (!pathExists) {
            console.error('Public key not found. Please generate a key pair first.');
            return;
        }
...
const signMessage = async (message) => {
    try {
        const pathExists = fs.existsSync(path.join(keyDir, 'private.pem'));
        if (!pathExists) {
            console.error('Private key not found. Please generate a key pair first.');
            return;
        }
